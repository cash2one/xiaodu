...
class GenSQLFromDB(object):
    ...
    def gen_init_normalizedLinkField_sql(self, file_path):
        ...
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        file_name = file_path + "movie_data_ori_update.txt"
        with open(file_name, "a+") as sql_file:
            ...
            for row in rows:
                ...
                sql_file.write(update_sql %(new_link, link_sign1, link_sign2))
    ...
    def gen_init_normalizedListLinkField_sql(self, file_path):
        ...
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        #comic
        file_name = file_path + "comic_data_ori_update.txt"
        with open(file_name, "a+") as sql_file:
            ...
            for row in rows:
                ...
                sql_file.write(update_sql %(new_list_link, list_link_sign1, list_link_sign2))
                
        #show
        file_name = file_path + "shows_data_ori_update.txt"
        with open(file_name, "a+") as sql_file:
        ...
        #tvplay
        file_name = file_path + "tvplay_data_ori_update.txt"
        with open(file_name, "a+") as sql_file:
...
if __name__ == '__main__':
    ...
    if gensql.init_db_handle():
        #for movie
        gensql.gen_init_normalizedLinkField_sql("./sql")
        #for tv,comic,show
        gensql.gen_init_normalizedListLinkField_sql("./sql")
        print 'DONE!!!'
