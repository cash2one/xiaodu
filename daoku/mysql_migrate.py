...
def process():
    try:
        for result in db._select2dic():
            result['site'] = result['app_name']
            result['horizontal_thumnail_url'] = result['thumnail_url']
            result['vertical_thumnail_url'] = ''
            result['down_count'] = 0
            result.pop('thumnail_url')
            result.pop('app_name')
            result.pop('state')
            result.pop('update_time')
            result.pop('insert_time')
            result.pop('id')

            if result['title'] == '':
                if u'，' in result['description']:
                    title_str = result['description'].split(u'，')[0]
                else:
                    title_str = result['description'][:200]
                if len(title_str) > 200:
                    title_str = result['description'][:200]
                result['title'] = title_str
            try:
                result1 = check_data.check(result)
                if result1['num'] == 0:
                    print result1['dict']
                    insert_db._insert(**result1['dict'])
                else:
                    print result1['error']
