...
class shoulu_api {
	...
	function processUrl($url){
		...
		/*
		$durations = split(":", $mtime->duration);
		if(count($durations)==2){
			$duH = 0;
			$duM = $durations[0];
			$duS = $durations[1];
		}else if( count($durations) ==3 ){
			$duH = $durations[0];
			$duM = $durations[1];
			$duS = $durations[2];
		}else if ( count($durations)==1 ){
			$duH = 0;
			$duM = 0;
			$duS = $mtime->duration;
		}else{
		...
		*/
		...
		$mitem->title = mysql_escape_string($mitem->title);
		...
		$cmtMainUpdate ="update tbl_video_$tblNumMain set categorys='$category', tags='$tag_mitem', album_name='$mitem->album', album_index='$mitem->albumIndex', title='$mitem->title', play_link='$mitem->url', real_link='$mitem->swf', file_type=1, image_link='$imgurl', duration=$duration, time_publish='$mitem->updateTime' , site = '$this->site', site_name = '$this->site_name', play_link_sign64='".$sign64."',  categorys_ori='$category_ori' where link_sign1=".$sign[0]." and link_sign2=".$sign[1]."  ;";
		...
		foreach($items as $item){
			...
			$cmtRelUpdate ="update tbl_video_$tblNumRel set categorys='$categoryRel', tags='$tag_item', album_name='$album_item', album_index='$item->albumIndex', title='$item->title', play_link='$item->url', real_link='$item->swf', file_type=1, image_link='$imgurlRel', duration=$duration, time_publish='$item->updateTime', site='$this->site', site_name = '$this->site_name', play_link_sign64='".$signRel64."', categorys_ori='$categoryRel_ori' where link_sign1=".$signRel[0]." and link_sign2=".$signRel[1].";";
